...
- **알림 품질 향상**
  - Promtail 레벨 기반 필터링(ERROR/WARN/FATAL/CRITICAL)으로 무의미한 알림은 제외하고 진짜 중요한 장애 로그에 집중합니다.
- **장애 대응 프로세스 자산화**
...
| rocky01 | 172.16.8.201 | Ollama, Loki, Grafana, Prometheus, handler.py |
| rocky02 | 172.16.8.202 | PostgreSQL |
...
2. 실시간 필터링 (Filter)
   Promtail이 ERROR/WARN/FATAL/CRITICAL 키워드만 필터링
